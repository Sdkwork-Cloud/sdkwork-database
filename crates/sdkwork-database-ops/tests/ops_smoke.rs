use std::sync::Arc;

use sdkwork_database_config::{DatabaseConfig, DatabaseEngine};
use sdkwork_database_lifecycle::LifecycleOrchestrator;
use sdkwork_database_ops::DatabaseOpsService;
use sdkwork_database_spi::DefaultDatabaseModule;
use sdkwork_database_sqlx::create_pool_from_config;
use tempfile::TempDir;

fn write_file(path: &std::path::Path, content: &str) {
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    std::fs::write(path, content).unwrap();
}

#[tokio::test]
async fn ops_status_after_migrate() {
    let temp = TempDir::new().unwrap();
    let root = temp.path();

    write_file(
        &root.join("database/database.manifest.json"),
        r#"{
  "schemaVersion": 1,
  "kind": "sdkwork.database.module",
  "moduleId": "demo",
  "serviceCode": "DEMO",
  "tablePrefix": "demo_",
  "contractVersion": "0.1.0",
  "paths": {
    "contract": "contract/schema.yaml",
    "migrations": "migrations",
    "seeds": "seeds",
    "driftPolicy": "drift/policy.yaml"
  },
  "lifecycle": { "activeSeedLocales": ["zh-CN"] }
}"#,
    );
    write_file(
        &root.join("database/migrations/sqlite/0001_create_probe.up.sql"),
        "CREATE TABLE demo_probe (id INTEGER PRIMARY KEY, label TEXT NOT NULL);",
    );

    let module = Arc::new(DefaultDatabaseModule::from_app_root(root).unwrap());
    let config = DatabaseConfig {
        engine: DatabaseEngine::Sqlite,
        url: "sqlite::memory:".to_string(),
        max_connections: 1,
        ..Default::default()
    };
    let pool = create_pool_from_config(config).await.unwrap();
    LifecycleOrchestrator::new(pool.clone(), module.clone())
        .migrate()
        .await
        .unwrap();

    let status = DatabaseOpsService::new(pool.clone(), module.clone())
        .status()
        .await
        .unwrap();
    assert_eq!(status.module_id, "demo");
    assert_eq!(status.drift_status, "clean");
    assert_eq!(status.pending_migrations, 0);

    let service = DatabaseOpsService::new(pool, module);
    let first = service.drift(false).await.unwrap();
    let second = service.drift(false).await.unwrap();
    assert_eq!(first.report.checked_at, second.report.checked_at);

    let refreshed = service.drift(true).await.unwrap();
    assert_ne!(first.report.checked_at, refreshed.report.checked_at);
}
